...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 22 15:29:29 2022

@author: emilypaulin
"""

#Binary model on responses to Ukraine closely follow question (12,533 responses to this particular question)
import sklearn
import pandas as pd
import numpy as np
import psycopg2
...
rutracker["ukraine_closely_follow"] = rutracker["ukraine_closely_follow"].astype(str).str.replace("1.0", "1")
rutracker["ukraine_closely_follow"] = rutracker["ukraine_closely_follow"].astype(str).str.replace("2.0", "1")
rutracker["ukraine_closely_follow"] = rutracker["ukraine_closely_follow"].astype(str).str.replace("3.0", "0")
rutracker["ukraine_closely_follow"] = rutracker["ukraine_closely_follow"].astype(str).str.replace("4.0", "0")
rutracker["ukraine_closely_follow"] = rutracker["ukraine_closely_follow"].astype(str).str.replace("5.0", "0")
rutracker["ukraine_closely_follow"] = rutracker["ukraine_closely_follow"].astype(str).str.replace("nan", "")
rutracker["ukraine_closely_follow"] = pd.to_numeric(rutracker["ukraine_closely_follow"], errors="coerce")
rutracker["survey_date"] = pd.to_datetime(rutracker["survey_date"])

support = pd.DataFrame(columns = ["survey_date","rgnid","gender","ukraine_closely_follow"])
support["survey_date"] = rutracker["survey_date"]
support["rgnid"] = rutracker["rgnid"]
support["gender"] = rutracker["gender"]
support["ukraine_closely_follow"] = rutracker["ukraine_closely_follow"]
support #5091 cells
...
aggposts.dropna(subset=["text", "ukraine_closely_follow"], inplace=True) #1083 now
...
train_df.to_csv(path_or_buf="followbtrain.csv") #758
test_df.to_csv(path_or_buf="followbtest.csv") #325

#Upload csvs to bucket and run on EC2
train_df = pd.DataFrame(columns=["text"])
for chunk in pd.read_csv("/home/ec2-user/followbtrain.csv", chunksize = 25000, encoding="utf-8"): #change depedning on system
    train_df = pd.concat([train_df, chunk])
    
del train_df["Unnamed: 0"]
train_df.dropna(subset=["text", "ukraine_closely_follow"], inplace=True)
...
train_df = pd.DataFrame(columns=["text"])
for chunk in pd.read_csv("/home/ec2-user/followbtrain.csv", chunksize = 25000, encoding="utf-8"):
    train_df = pd.concat([train_df, chunk])
...
del train["sex"]
del train["rgnid"]
del train["ukraine_closely_follow"] #Leaving the text column
...
for chunk in pd.read_csv("/home/ec2-user/followbtest.csv", chunksize = 25000, encoding="utf-8"):
    test_df = pd.concat([test_df, chunk])
    
del test_df["Unnamed: 0"]    
test_df.dropna(subset=["text", "ukraine_closely_follow"], inplace=True)    
test = test_df #85
del test["date"]
del test["sex"]
del test["rgnid"]
del test["ukraine_closely_follow"]
...
embedding_model = keras.models.load_model("/home/ec2-user/environment/embedding_model2")
batch_size = 200
...
pd.DataFrame(predictions).to_csv(path_or_buf="followbpredictions.csv") 
