...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 18 14:14:45 2022

@author: emilypaulin
"""

#This is a document of many different attempts to get the categorical cross entropy model to work. 
#Query down full data and redo average of support by census cell.
...
print(supportbycell)
#Now go down to necessary section
...
aggregatedposts = aggregatedposts.merge(supportbycell, left_on=["date","rgnid","sex"], right_on=["survey_date","rgnid","gender"])
del aggregatedposts["survey_date"]
del aggregatedposts["gender"]
...
train_df.to_csv(path_or_buf="smalltrain.csv")
test_df.to_csv(path_or_buf="smalltest.csv")
...
s3_client.download_file('sm-support-model', 'smalltrain.csv', 'smalltrain.csv')
s3_client.download_file('sm-support-model', 'smalltest.csv', 'smalltest.csv')
...
for chunk in pd.read_csv("smalltest.csv", chunksize = 25000, encoding="utf-8"):
...
supportbycell = rutracker.groupby(["survey_date", "rgnid"])["ukraine_approval"].mean()
supportbycell = supportbycell.reset_index()
print(supportbycell)
...
df2 = joinedposts.groupby(["date", "rgnid"])["text"].apply(lambda x: ','.join(x))
aggregatedposts = df2.reset_index()
aggregatedposts["date"] = pd.to_datetime(aggregatedposts["date"])
aggregatedposts = aggregatedposts.merge(supportbycell, left_on=["date","rgnid"], right_on=["survey_date","rgnid"])
del aggregatedposts["survey_date"]

train_df, test_df = sklearn.model_selection.train_test_split(aggregatedposts, test_size=0.3, train_size=0.7, random_state=None, shuffle=True, stratify=None)
train_df.to_csv(path_or_buf="medtrain.csv")
test_df.to_csv(path_or_buf="medtest.csv")

for object in bucket.objects.all():
          print(object.key)

s3_client.download_file('sm-support-model', 'medtrain.csv', 'medtrain.csv')
s3_client.download_file('sm-support-model', 'medtest.csv', 'medtest.csv')

#training
train_df = pd.DataFrame(columns=["text"]) #197
for chunk in pd.read_csv("medtrain.csv", chunksize = 25000, encoding="utf-8"):
    train_df = pd.concat([train_df, chunk]) 
...
#testing
test_df = pd.DataFrame(columns=["text"])
for chunk in pd.read_csv("medtest.csv", chunksize = 25000, encoding="utf-8"):
    test_df = pd.concat([test_df, chunk])
...
pd.DataFrame(predictions).to_csv(path_or_buf="realbinarypredictions.csv")

#
# Binary classification model (use other embedding model, not original)
#
rutracker = pd.DataFrame()
for chunk in pd.read_csv("/home/ec2-user/environment/rutracker.csv", chunksize=25000, encoding="utf-8"):
    rutracker = pd.concat([rutracker, chunk])
...
aggposts.dropna(subset=["text", "ukraine_approval"], inplace=True) #1083 now
...
train_df.to_csv(path_or_buf="binarytrain.csv") #758
test_df.to_csv(path_or_buf="binarytest.csv") #325

train_df = pd.DataFrame(columns=["text"])
for chunk in pd.read_csv("/home/ec2-user/binarytrain.csv", chunksize = 25000, encoding="utf-8"): #change depedning on system
    train_df = pd.concat([train_df, chunk])
...
support_arr
train_df = pd.DataFrame(columns=["text"])
for chunk in pd.read_csv("/home/ec2-user/binarytrain.csv", chunksize = 25000, encoding="utf-8"):
    train_df = pd.concat([train_df, chunk])
del train_df["Unnamed: 0"]
train = train_df
del train["date"]
...
test_df = pd.DataFrame(columns=["text"])
for chunk in pd.read_csv("/home/ec2-user/binarytest.csv", chunksize = 25000, encoding="utf-8"):
    test_df = pd.concat([test_df, chunk])
del test_df["Unnamed: 0"]    
